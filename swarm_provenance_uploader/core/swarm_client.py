...
def purchase_postage_stamp(gateway_url: str, amount: int, depth: int) -> str: # PARAMETER ORDER SWAPPED HERE
     """
    Purchases a new postage stamp from the Bee Gateway.
    Parameters are in the order {amount}/{depth} as per Bee API.
    Returns the Stamp ID (batchID).
     """
     headers = {"Content-Type": "application/json"}
     # Bee API uses {amount}/{depth}
     api_path = f"/stamps/{amount}/{depth}" # CORRECTED ORDER
     url = urljoin(gateway_url, api_path)

     # For debugging the URL:
     # print(f"DEBUG: Stmap Purchase URL: {url}")
     ...
     try:
        ...
        return stamp_id
         ...
         raise ConnectionError(f"Stamp purchase failed for URL {url}: {e}") from e # Added URL to error
     except (ValueError, KeyError) as e:
         # Wrap JSON parsing/content errors
        raise ValueError(f"Could not parse stamp purchase response from URL {url}: {e}") from e # Added URL to error
...
def upload_data(gateway_url: str, data_to_upload: bytes, stamp_id: str, content_type: str = "application/json") -> str:
    ...
    try:
        ...
        return swarm_hash
         ...
         # Wrap network/http errors
         raise ConnectionError(f"Swarm upload failed for URL {url}: {e}") from e # Added URL to error
    except (ValueError, KeyError) as e:
         # Wrap JSON parsing/content errors
        raise ValueError(f"Could not parse upload response from URL {url}: {e}") from e # Added URL to error
